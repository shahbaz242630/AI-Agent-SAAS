/**
 * How the mailbox capability hands a new lead to whoever cares (slice 3.1c-3).
 *
 * 🚨 THIS EXISTS BECAUSE THE CAPABILITY MUST NEVER IMPORT A PRODUCT, AND
 ...
 * the product owns the reply). A direct call from `inbound-intake.service.ts`
 * to `LeadReplyService` would be `capabilities/ → products/`, which
 ...
 * in the capability with the implementation in the product, wired at the
 * composition root — the `MAIL_PROVIDERS` pattern. This is that, for a handoff
...
export interface NewLeadHandler {
   ...
   * answer 200. An exception escaping here would fail the webhook, Resend would
   * retry it, and intake's idempotency would correctly refuse to make a second
   * lead — so the enquiry would be safe but the retry storm would be pointless.
   * Worse, a handler that throws on every delivery would keep the whole
   * pipeline red for a fault in one product.
   *
   * The dispatcher below enforces this by catching, but a handler that relies
   * on being caught is a handler nobody can reason about. Own your failures and
   * record them.
   */
