...
| Required approvals              | **0 — INTERIM**                           | Bump to **1 (founder)** the day the builder bot account exists (G-003)                       |
...
**Still open:** builder bot account (founder action, G-003) — then required approvals = 1 + code-owner review switches on; pnpm ≥ 10.16 + `minimumReleaseAge`; ZAP (needs staging, Slice 0.4).
...
| G-003 | 2026-07-22 | Security review       | Builder agent commits under founder's identity — move to bot account, then set required approvals = 1 + code-owner review                    | High     | Before next slice               | Open — awaiting founder                                                                                            |
...
| G-006 | 2026-07-22 | Sandoq Kin comparison | Supabase on free tier has no managed backups — manual off-site `pg_dump` required before hosted schema work; Pro upgrade before staging/prod | High     | Before Slice 0.4 staging deploy | Open                                                                                                               |
...
- **Next (founder):** create builder bot account → builder bumps ruleset to 1 approval + code-owner review (G-003).
...
- **While Supabase stays on the free tier:** take and verify a manual off-site logical backup (`pg_dump`) before any material hosted schema work; upgrade to Supabase Pro before staging/production use (G-006).
